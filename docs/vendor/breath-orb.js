...
(function (global) {
  ...
  var FS = [
    ...
    'vec2 p=v*2.-1.;',
    'float r=length(p);',
    'float rad=.80+.10*uBreath;',
    'float d=rad-r;',
    'if(d<-.08){gl_FragColor=vec4(0.);return;}',
    'float soft=smoothstep(-.04,.22,d);',
    'float core=smoothstep(.55,.0,r/rad);',
    'float fres=pow(1.-clamp(r/rad,0.,1.),1.8);',
    'float n=noise(p*3.2+uT*.15)+noise(p*7.+uT*.08)*.5;',
    'vec3 col=mix(uAccent*.55,uAccent,soft*.9+n*.1);',
    'col=mix(col,uAccent*1.05,core*.45);',
    'col+=uAccent*fres*.65;',
    'float a=soft*(.78+.22*uLum)+core*.18;',
